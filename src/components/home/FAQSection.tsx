      ...
      <div className="relative z-10 mx-auto w-full max-w-2xl px-6 md:max-w-[1320px] md:px-10 lg:px-14">
        <div className="hidden items-stretch md:grid md:grid-cols-[180px_minmax(0,1fr)] md:gap-10 md:py-4 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-14">
          <div className="flex h-full flex-col justify-between py-2">
            ...
            <div className="mt-10 max-w-[165px] lg:max-w-[190px]">
              <p className="text-[12px] leading-relaxed text-white/90 lg:text-[13px]">
                Have More Questions? We&apos;re Happy To Help!
              </p>
              <Link
                href="/contact-us"
                className="mt-5 inline-flex items-center gap-2 text-[15px] font-medium uppercase tracking-wide text-white hover:text-white/90 lg:text-[17px]"
              >
                CONTACT US
                <svg
          ...
          <div className="relative py-2">
            <div className="absolute bottom-2 right-0 top-2 w-px bg-white/18" />
            <div className="pr-10 lg:pr-12">
              {faqs.map((item, index) => {
                ...
                return (
                  <div key={`desktop-${item.id}`} className="border-b border-white/35">
                    <button
                      type="button"
                      onClick={() => setOpenIndex(isOpen ? null : index)}
                      className="flex w-full items-center justify-between gap-5 py-4 text-left text-white focus:outline-none lg:py-[15px]"
                      aria-expanded={isOpen}
                      aria-controls={`faq-answer-desktop-${item.id}`}
                      id={`faq-question-desktop-${item.id}`}
                    >
                      <span className="text-[15px] font-normal leading-snug text-white lg:text-[16px]">
                        {question}
                      </span>
                      <span
                    ...
                    {hasAnswer && (
                      <div
                        id={`faq-answer-desktop-${item.id}`}
                        role="region"
                        aria-labelledby={`faq-question-desktop-${item.id}`}
